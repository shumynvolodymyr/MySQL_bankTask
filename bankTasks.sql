...
join client c ON idDepartment = c.Department_idDepartment
join application a ON c.idClient = a.Client_idClient
...
join application a ON a.Client_idClient = idClient
...
select idClient,concat(FirstName," ", LastName) as FullName, City,  count(Client_idClient) as 'К-сть кредитів' 
from client
join application a ON idClient = a.Client_idClient
group by Client_idClient
order by count(Client_idClient) desc
